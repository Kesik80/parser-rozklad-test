// api/fetch-timetable-sw.js
// Джерело: swrailway.gov.ua (через ScrapingBee)
// GET /api/fetch-timetable-sw?tid=28320
...
export default async function handler(req, res) {
  ...
  const cacheKey = `sw:timetable:${tid}`;
...
function parseTimetable(html, tid) {
  ...
  while ((m = rowRegex.exec(html)) !== null) {
    ...
    let td;
    while ((td = tdRegex.exec(row)) !== null) tds.push(cleanTd(td[1]));
    ...
    const arr = isTime(tds[2]) ? tds[2] : null;
    const dep = isTime(tds[3]) ? tds[3] : null;
    if (arr || dep) stations.push({ name, arr, dep });
  }
